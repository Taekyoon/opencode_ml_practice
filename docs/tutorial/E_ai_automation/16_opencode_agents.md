...
이 프로젝트의 `.opencode/agents/`에 두 에이전트가 있다:
...
`semiconductor-failure-predictor.md` 예:
...
| `mode` | `subagent`(용도별 분리 실행) / `main`(코어 대화) |
| `permission` | read/edit/bash/skill 등 도구 허용 범위 |
...
```
frontmatter의 `mode`와 `permission`가 어떻게 다른지 관찰한다.
...
2. `mode: subagent`와 `main`의 차이는?
3. 에이전트를 "너무 많은 걸 하는 종합 에이전트"로 만들면 왜 안 좋은가?

## opencode에게 물어보세요

```
.opencode/agents/ 폴더의 두 에이전트를 읽고, 각각 언제 누가 부르면 되는지
비교 정리해줘. 그리고 내가 우리 프로젝트에 맞는 에이전트를 하나 더 만들고 싶으면
어떤 걸 만들면 좋을지 추천해줘.
```

## 다음 레슨
[E2. 스킬 만들기](17_creating_skills.md) — 반복 문제 해결을 위한 전문 스킬을 만든다.