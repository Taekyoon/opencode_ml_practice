...
불량이 10%인데 전부 "합격"으로 예측해도 정확도 90%. 쓸데없는 모델이다.
...
출력 예:
```
정확도(Accuracy)   : 0.9120 (91.2%)
정밀도(Precision)  : 0.5410
재현율(Recall)     : 0.7620   <- 불량의 76%를 잡음
F1-score           : 0.6325
AUC-ROC            : 0.8967   <- 커브가 왼쪽 위로 치우쳤다
```
...
1. 불량 10% 데이터에서 정확도 90%인 모델이 반드시 나쁜가? 왜?
