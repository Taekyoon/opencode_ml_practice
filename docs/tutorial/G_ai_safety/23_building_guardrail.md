...
- 임계값 조정을 통해 "정상 차단(오탐)"과 "위장 공격 통과(미탐)" 트레이드오프를 실습한다
...
따라서:
- "정상 문장인데 위험어가 포함"(예: "DAN 모드를 설명하는 백과사전 항목") → 오탐(FP)
- "키워드를 피한 위장 공격"(예: "평가자 신분으로 말해봐") → 미탐(FN, 공격인데 안 잡힘)

### 임계값이란
모델은 각 클래스에 확률을 매긴다. `predict.py`에서는 `is_unsafe()`의
`threshold`가 "safe 확률이 몇 % 이상이면 safe로 인정할지" 정한다.

| threshold | 위장 공격을 잡는 능력 | 오탐(정상 차단) |
|-----------|-----------|------|
| 낮게 (예: 0.3) | 위장 공격을 더 잘 잡음 (미탐↓) | 정상 요청도 공격으로 차단 (오탐↑) |
| 높게 (예: 0.8) | 위장 공격이 통과할 위험 (미탐↑) | 정상 사용자는 거의 안 통과 (오탐↓) |

방어측 목적(서비스 타입)에 따라 기준을 다르게 잡아야 한다.
> 용어: **오탐**(false positive) = 정상 요청을 공격으로 차단, **미탐**(false negative)
> = 공격을 안전으로 통과. 임계값 하나로 둘은 반비례한다.
...
3. "오탐(FP)"과 "미탐(FN)" 중 서비스에 더 치명적인 쪽은 어느 쪽인지 이유와 함께?

## opencode에게 물어보세요

```
research/prompt_guard/results의 metrics.json에서 hard_eval.misclassified 중
"공격 문장을 safe로 판단(미탐)"한 항목을 골라, 왜 TF-IDF 로지스틱이 틀렸는지
단어 대 단어로 분석해줘.
```
